...
def askPlayerName():
    ...
    canvas1.create_text( screen_width/4,screen_height/7.5, text = "Enter Name:", font=("Chalkboard SE",60), fill="black")
    ...
    button = Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=9, command=saveName, height=1, bg="grey", bd=3)
    button.place(x = screen_width/5.5, y=screen_height/3)
    ...
    for row in range(0,3):
        ...
        while (counter < len(randomColList)):
            ...
            if(randomNumber not in currentNumberList):
                ...
                numberBox.configure(text=randomNumber, fg="#2e1c26",bg="#FF835A")
